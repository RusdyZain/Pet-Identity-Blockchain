...
export const ClinicNotificationsPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message ?? 'Gagal memuat notifikasi klinik');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      />
      {loading && <Loader label="Memuat notifikasi klinik..." />}
      ...
      <div className="space-y-3">
        ...
        {!loading && notifications.length === 0 && (
          <p className="text-sm text-slate-500">Tidak ada notifikasi untuk klinik.</p>
        )}
      </div>
    </div>
  );
