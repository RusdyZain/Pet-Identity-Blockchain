...
exports.verifyMedicalRecordController =
  exports.listPendingRecordsController =
  exports.listMedicalRecordsController =
  exports.createMedicalRecordController =
    void 0;
...
const listMedicalRecordsController = async (req, res, next) => {
  try {
    if (!req.user) throw new errors_1.AppError("Unauthorized", 401);
    const petId = Number(req.params.petId);
    const records = await (0, medicalRecordService_1.listMedicalRecords)(
      petId,
      req.user
    );
    res.json(records);
...
const listPendingRecordsController = async (req, res, next) => {
  try {
    ...
    medicalRecordService_1.listPendingRecordsForClinic)(req.user.id);
    res.json(records);
  } catch (error) {
    next(error);
  }
...
const verifyMedicalRecordController = async (req, res, next) => {
  try {
    ...
    if (!status) throw new errors_1.AppError("Status wajib diisi", 400);
    const updated = await (0, medicalRecordService_1.verifyMedicalRecord)(
      recordId,
      req.user.id,
      status
    );
